...
const AdminPanel: React.FC<AdminPanelProps> = ({ onLogout }) => {
  ...
  const fetchUsers = (pageNum: number) => {
      setLoading(true);
      fetch(`http://localhost:5000/api/admin/users?page=${pageNum}&limit=10`)
      .then(res => res.json())
      .then(data => {
          setUsers(data.users);
  ...
  const approvePayment = async (requestId: string) => {
      ...
      try {
          const res = await fetch('http://localhost:5000/api/admin/approve-payment', {
              method: 'POST',
  ...
  const rejectPayment = async (requestId: string) => {
      ...
      try {
          const res = await fetch('http://localhost:5000/api/admin/reject-payment', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ requestId })
          });
  ...
  const handleWalletUpdate = async () => {
      ...
      try {
          await fetch('http://localhost:5000/api/admin/update-user-wallet', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ 
  ...
  const handleChangePassword = async (e: React.FormEvent) => {
      ...
      try {
          await fetch('http://localhost:5000/api/admin/change-password', {
              method: 'PUT',
              headers: { 'Content-Type': 'application/json' },
