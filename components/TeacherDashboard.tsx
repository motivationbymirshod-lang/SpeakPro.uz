...
const TeacherDashboard: React.FC<TeacherDashboardProps> = ({ user: initialUser, onLogout }) => {
  ...
  useEffect(() => {
    const refreshData = async () => {
         try {
             ...
             const uRes = await fetch(`http://localhost:5000/api/user/${initialUser.email}`);
             if(uRes.ok) {
             ...
             // 2. Refresh Students
             const sRes = await fetch(`http://localhost:5000/api/teacher/students?teacherId=${initialUser._id || initialUser.id}`);
             if (sRes.ok) {
  ...
  const handleAddStudent = async (e: React.FormEvent) => {
      e.preventDefault();
      try {
          const res = await fetch('http://localhost:5000/api/teacher/create-student', {
              method: 'POST',
          ...
          if (res.ok) {
              ...
              // Refresh List
              const sRes = await fetch(`http://localhost:5000/api/teacher/students?teacherId=${user._id || user.id}`);
              const sData = await sRes.json();
              setStudents(sData);
          } else {
              alert(data.message);
  ...
  const handleDistributeCredit = async (studentId: string) => {
      ...
      try {
          const res = await fetch('http://localhost:5000/api/teacher/distribute-credit', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
